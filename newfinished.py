...
def process_tv(file_path, show_id, season_number, episode_number, crop=False, eng_only=True):
  ...
  if os.path.splitext(file_path)[1].lower() in ['.mkv', '.mp4', '.avi']:
    ...
    show_name_safe = ' '.join(illegal_chars.sub(repl=' ', string=show_name).split()).replace(u'\u2019',r"'")
    if show_name_safe[-1] == '.':
    ...
    with Cleaner('{:s} S{:02d}E{:02d}'.format(show_name_safe, season_number, episode_number), id) as c:
      ...
      episode_name_safe = ' '.join(illegal_chars.sub(repl=' ', string=episode_name).split()).replace(u'\u2019',r"'")
...
def on_remove_torrent(success):
  if success:
    log.info('Torrent and data successfully removed.')
  else:
    log.warn('Torrent and data could not be removed.')
    ...
    if tag:
      ...
      if len([f for f in torrent_files if f['path'][-3:].lower() == 'rar']) > 0:
        ...
        for f in [f for f in torrent_files if f['path'][-3:].lower() == 'rar']:
          with Timer('Reading rar file', id):
            rar = rarfile.RarFile(os.path.join(torrent_folder, f['path']))
          video_files_in_rar = sorted([rf for rf in rar.infolist() if rf.filename[-3:].lower() in ['mkv', 'mp4', 'avi']], key=lambda rf: rf.file_size, reverse=True)
          with Timer('Extracting', id) as t:
        ...
        video_files_in_torrent = sorted([f for f in torrent_files if f['path'][-3:].lower() in ['mkv', 'mp4', 'avi']], key=lambda f: f['size'], reverse=True)
        ...
        cleanup()
        return
      target = sorted([d for d in targets], key=lambda d: d['size'], reverse=True)[0]
      ...
      log.debug('Label \'{:s}\' not recognized'.format(label))
      cleanup()
  else:
    log.debug('Torrent is not labeled')
    cleanup()

def on_get_status_failed(result):
  log.error('Failed to get torrent status = {}'.format(repr(result)))
  cleanup()
  return

def on_connect_success(result):
  log.debug('Connected to daemon with result {:d}'.format(result))
  log.debug ('Fetching torrent status for id {:s}...'.format(torrentId))
...
temp_files = []
torrentId = argv[1]
log.info('finished.py called on {:s}'.format(argv[2]))
log.debug('Waiting ten seconds...')
sleep(10)
log.debug('Connecting to daemon...')
d = client.connect(username='flexget', password='flexgetdeluge')
